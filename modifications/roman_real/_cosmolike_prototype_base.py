...
class _cosmolike_prototype_base(DataSetLikelihood):
  ...
  def initialize(self, probe):
    ...
    self.pc_file = ini.relativeFileName('pc_file')
    self.U = np.genfromtxt(self.pc_file)[:,:self.npc] #DHFS MOD
  ...
  def set_source_related(self, **params_values):
    ...
    if self.external_nz_modeling: 
      ...
      pca_obj = nz_pca.PCA(source_nz_local,survey,self.npc)
      nz_mod = pca_obj.pca(params_values,self.U)
      ci.set_source_sample(nz_mod)
  ...
  #DHFS MOD START - FROM lsst y1 prototype
  ...
  #DHFS MOD END - FROM lsst y1 prototype