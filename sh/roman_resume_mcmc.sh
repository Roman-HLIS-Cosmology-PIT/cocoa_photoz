#!/bin/bash -l
#SBATCH --job-name=PCA_diff_gauss
...
yaml_file=cocoa_photoz/yamls/roman_sc1bd4_g_18decimal_diff_params_than_cosmocov_gauss_alphas_prior/PCA_MODEL_MCMC${SLURM_ARRAY_TASK_ID}.yaml
...
$CONDA_PREFIX/bin/mpirun -n ${SLURM_NTASKS} --oversubscribe --mca pml ^ucx --mca btl vader,tcp,self --bind-to core:overload-allowed --rank-by slot --map-by numa:pe=${OMP_NUM_THREADS} cobaya-run $yaml_file -r
