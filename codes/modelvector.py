...
def data_file_f(survey,sc=''):
    if (survey==surveys[0] or survey==surveys[1] or survey==surveys[2]):
        data_files = {'lsst_y1':'lsst_y1_M1_GGL0.05.dataset',
                     'des_y3':'des_y3_real.dataset',
                     'roman_real':'roman_sc1bd4_g.dataset',
                    }
        return data_files[survey]
    elif (survey == surveys[3]):
        return f'roman_{sc}.dataset'

def modelvector(survey,sc=''):
    data_file = data_file_f(survey,sc)
    ############ GENERAL PARAMETERS ############
    non_linear_emul       = 2
    IA_model              = 0
...
modelvector(surveys[2])

# for sc in scenarios:
#     data_file = data_file_f(surveys[3],sc)