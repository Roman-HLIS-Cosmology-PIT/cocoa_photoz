...
parser.add_argument('--npcs_used', type=int, required=True, help='Number of PCs')
parser.add_argument('--weights', type=int, required=True, help='Use Fisher weights')
...
def projection(encoding,decoding,plot=False):
    ...
    if plot:
        for i in range(1,10):
            plt.plot(z,nz_fid[:,i],c="red",label="fid" if i==1 else None,lw="3",alpha=.4)
            plt.plot(z,nbar[:,i],c="gray",ls=":",label="mean" if i==1 else None)
            # plt.plot(z,nz_model[:,i],c="darkgreen",ls=":",label="recon" if i==1 else None)

        plt.legend(loc="lower center",ncols=2,fontsize=10,bbox_to_anchor=(0.5, 1.0),frameon=False)
        plt.xlabel(r"$z$")
        plt.ylabel(r"$n(z)$")
        plt.savefig(f"nz_r{sim}_npcs{npcs_used}_weights{weights}.pdf")
...
def execute_end_to_end_pipeline(extra_info=False):
    if weights == False:
        print("!!-------ATTENTION--------!!") 
        print("!!-------ATTENTION--------!!") 
        print(f"NOT USING FISHER: weights={weights}")
        print("!!-------ATTENTION--------!!") 
        print("!!-------ATTENTION--------!!") 
    F = fisher(weights=weights)
    E, D, ss, resid, Dvec, Dval, Cnvec, Cnval, Um, Vm, M = roman_obj.getModes(F, Cn, chisq_threshold=0)
    projection(encoding=E,decoding=D,plot=True)
    ...
    if extra_info:
        ...
        np.savetxt(f"E_r{sim}_weights{weights}.txt",E)
        np.savetxt(f"D_r{sim}_weights{weights}.txt",D)
        np.savetxt(f"X_r{sim}_weights{weights}.txt",X)
        np.savetxt(f"Cnvec_r{sim}_weights{weights}.txt",Cnvec)
        np.savetxt(f"test_r{sim}_weights{weights}.txt",test)
    return None
