...
def compute_alpha_distribution(compute_simple=False,compute_specific=False,plot_simple=False,plot_specific=False):
    ...
    if plot_simple==True and plot_specific==False:
        ...
        g.triangle_plot([chains_mm,
                         ...
                         chains_fm],
                        ["projected_alpha%s" %i for i in range(5)],
                        # legend_labels=[f"{rename_scenarios[sc_mod]}",
                        #                f"{rename_scenarios[sc_fid]}",
                        #                f"{rename_scenarios[sc_mod]}" r"$\times$" f"{rename_scenarios[sc_fid]}",
                        #                f"{rename_scenarios[sc_fid]}" r"$\times$" f"{rename_scenarios[sc_mod]}"
                        #                ],
                        legend_labels=[r"(1, 1)",
                                       r"(2, 2)",
                                       r"(1, 2)",
